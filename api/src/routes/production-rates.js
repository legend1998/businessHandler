...
router.get("/:location_id", async (req, res) => {
  ...
  const production_rates = await db("erp.production_rate").where(
    "location_id",
...
router.post(
  "/create",
  checkRequiredPOST("location_id", "item_id", "frequency", "target_amount"),
  async (req, res) => {
    const { location_id, item_id, frequency, target_amount } = req.body;

    // Validate fields
    if (
      !isValidNumber(target_amount) ||
...
router.post(
  "/delete-multiple",
  checkRequiredPOST("production_rate_ids"),
  async (req, res) => {
    // Data from body
    ...
    if (!Array.isArray(production_rate_ids))
      return res
        ...
        .send("production_rate_list_format_mismatch");

    // Get location list and check all locations exist initially
    const procution_rates = await db("erp.production_rate").whereIn(
      "id",
      production_rate_ids.map((id) => String(id))
    );
    if (

      production_rate_ids.sort().join(",") !==
      production_rate_ids
        .map((u) => u.id)
        .sort()
        .join(",")
    )
      return res.status(200).send("no_such_production_rates");

    // Delete locations from database
    await db("erp.production_rate")
      .whereIn(
        "id",
        procution_rates.map((u) => u.id)
      )
      .update({
        deleted: true,
      });
