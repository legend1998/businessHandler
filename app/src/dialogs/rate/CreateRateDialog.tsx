...
interface CreateRateDialogProps {
  open: boolean;
  onclose: () => void;
  onCreateRate: ( data:ProductionRate, callback: () => void) => void;
}

function CreateRateDialog(props: CreateRateDialogProps) {
  const { open, onclose, onCreateRate } = props;
  ...
  function createRateNow(callback:()=>void) {
